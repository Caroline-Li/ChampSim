...
void SPP_dev2::invoke_prefetcher(uint64_t ip, uint64_t addr, uint8_t cache_hit, uint8_t type, uint64_t instr_id, uint64_t curr_cycle,
                                 vector<uint64_t>& pref_addr, vector<bool>& pref_level, std::vector<int>& offsets)
{
  uint64_t page = addr >> LOG2_PAGE_SIZE;
  uint32_t page_offset = (addr >> LOG2_BLOCK_SIZE) & (PAGE_SIZE / BLOCK_SIZE - 1), last_sig = 0, curr_sig = 0, confidence_q[m_parent_cache->MSHR_SIZE],
           depth = 0;

  int32_t delta = 0, delta_q[m_parent_cache->MSHR_SIZE];

  for (uint32_t i = 0; i < m_parent_cache->MSHR_SIZE; i++) {
    confidence_q[i] = 0;
    delta_q[i] = 0;
  }
  confidence_q[0] = 100;
  ...
  do {
    ...
    while ((i < pf_q_tail) && (knob::spp_dev2_max_degree == 0 || pref_addr.size() < knob::spp_dev2_max_degree)) {
      if (confidence_q[i] >= knob::spp_dev2_pf_threshold) {
        ...
        if ((addr & ~(PAGE_SIZE - 1)) == (pf_addr & ~(PAGE_SIZE - 1))) { // Prefetch request is in the same physical page
          ...
          if (knob::spp_dev2_pf_l2_only) {
            ...
            pf_level = (confidence_q[i] >= knob::spp_dev2_fill_threshold);
          }
          ...
          if (FILTER.check(pf_addr, spp_level, GHR)) {
            ...
            offsets.push_back(delta_q[i]);

            // // Use addr (not base_addr) to obey the same physical page boundary
            // m_parent_cache->prefetch_line(ip, addr, pf_addr, pf_level, 0);

            stats.pref.total++;
            if (spp_level == SPP_L2C_PREFETCH)
              stats.pref.at_L2++;
            else
              stats.pref.at_LLC++;

            if (spp_level == SPP_L2C_PREFETCH || (knob::spp_dev2_pf_llc_only && spp_level == SPP_LLC_PREFETCH)) {
              GHR.pf_issued++;
              if (GHR.pf_issued > GLOBAL_COUNTER_MAX) {
                GHR.pf_issued >>= 1;
                GHR.pf_useful >>= 1;
              }
              SPP_DP(cout << "[ChampSim] SPP prefetch issued GHR.pf_issued: " << GHR.pf_issued << " GHR.pf_useful: " << GHR.pf_useful << endl;);
            }
...
void SPP_dev2::register_fill(uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint64_t curr_cycle)
{
#ifdef FILTER_ON
  SPP_DP(cout << endl;);
