...
void Streamer::invoke_prefetcher(uint64_t pc, uint64_t address, uint8_t cache_hit, uint8_t type, uint64_t instr_id, uint64_t curr_cycle,
                                 std::vector<uint64_t>& pref_addr, std::vector<bool>& pref_level, std::vector<int>& offsets)
{
    ...
    if(dir_match)
    {
        ...
        for(uint32_t index = 0; index < knob::streamer_pref_degree; ++index)
        {
            ...
            int real_pref_offset = (dir == +1) ? 1 : -1;
            if(pref_offset >= 0 && pref_offset < 64)
            {
                ...
                offsets.push_back(real_pref_offset);
            }
            else
            {
                break;
...
void Streamer::register_fill(uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint64_t curr_cycle) {
    return;
}