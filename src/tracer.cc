...
void Tracer::add_new_access(uint64_t signature, bool prediction, bool hit, uint32_t type, uint64_t core_clock_cycle, uint64_t ip) {
    ...
    if (type == PREFETCH) { // prefetch
        ...
        interval_map[signature].prefetches_in_accesses++;
...
void Tracer::print_pc_freq_map_prefetch(void) {
    ...
    std::cout << "num pcs to reach 0.9 pc frequency: " << num_pcs << std::endl;
}
...
void Tracer::print_accesses(std::vector<Access> v) {
    for (int i = 0; i < v.size(); i++) {
        if (v[i].type == Type::prefetch
            && !v[i].hit) {
            std::cout << "P_m ";
        }
        else if (v[i].type == Type::prefetch && 
            v[i].hit) {
            std::cout << "P_h ";
        }
        else if (v[i].type == Type::demand &&
            !v[i].hit) {
            std::cout << "D_m ";
        }
        else if (v[i].type == Type::demand &&
...
void Tracer::generate_prefetcher_accuracy(void) {
    int num_pm_ph_d_intervals = 0;
    int num_pm_ph_e_intervals = 0;

    int num_useful_prefetches = 0;
    int num_useless_prefetches = 0;
    for (auto& item : interval_map) {
        int num_useful_prefetches_accesses = 0;
        int num_useless_prefetches_accesses = 0;
        bool prefetch_line_filled = false;
        bool demand_line_filled = false;
        int num_prefetch_hits_in_prefetch_line = 0;
        int num_prefetch_hits_in_demand_line = 0;
        bool prefetch_useful = false;
        bool last_useful = false;
        bool last_prefetch = false;
        bool continuing_interval = false;
        for (int i = 0; i < item.second.current_index+1; i++) {
            if (item.second.accesses[i].type == Type::prefetch) {
                ...
                if (!item.second.accesses[i].hit) {
                    prefetch_line_filled = true;
                    continuing_interval = false;
                }
                else {
                    if (prefetch_line_filled) {
                        num_prefetch_hits_in_prefetch_line++;
                    }
                    else if (demand_line_filled) {
                        num_prefetch_hits_in_demand_line++;
                    }
                    else {
                        continuing_interval = true;
                        if (last_prefetch && last_useful) {
                            prefetch_line_filled = true;
                            prefetch_useful = true;
                            num_prefetch_hits_in_prefetch_line++;
                        }
                        else if (last_prefetch) {
                            prefetch_line_filled = true;
                            num_prefetch_hits_in_prefetch_line++;
                        }
                        else {
                            demand_line_filled = true;
                            num_prefetch_hits_in_demand_line++;
                        }
                    }   
                }

                // prefetch - end intervals
                if (!continuing_interval && (i == item.second.current_index) && prefetch_line_filled && !prefetch_useful) {
                    prefetch_line_filled = false;
                    num_pm_ph_e_intervals++;
                    num_useless_prefetches_accesses += num_prefetch_hits_in_prefetch_line + 1;
                    num_prefetch_hits_in_prefetch_line = 0;
                }
                else if ((i == item.second.current_index) && prefetch_line_filled) {
                    prefetch_line_filled = false;
                    num_useless_prefetches_accesses += num_prefetch_hits_in_prefetch_line;
                    num_prefetch_hits_in_prefetch_line = 0;
                    prefetch_useful = false;
                    continuing_interval = false;
                }
                else if ((i == item.second.current_index) && demand_line_filled) {
                    num_useless_prefetches_accesses += num_prefetch_hits_in_demand_line;
                    num_prefetch_hits_in_demand_line = 0;
                    demand_line_filled = false;
                    continuing_interval = false;
                }
                ...
                if (!continuing_interval && prefetch_line_filled && !prefetch_useful) {
                    num_pm_ph_e_intervals++;
                    prefetch_line_filled = false;
                    num_useless_prefetches_accesses += num_prefetch_hits_in_prefetch_line + 1;
                    num_prefetch_hits_in_prefetch_line = 0;
                ...
                if (!continuing_interval && prefetch_line_filled && !prefetch_useful) {
                    num_pm_ph_d_intervals++;
                    num_useful_prefetches_accesses += num_prefetch_hits_in_prefetch_line + 1;
                    prefetch_useful = true;
                    num_prefetch_hits_in_prefetch_line = 0;
                }
                else if (prefetch_line_filled) {
                    num_useful_prefetches_accesses += num_prefetch_hits_in_prefetch_line;
                    num_prefetch_hits_in_prefetch_line = 0;
                }
                else if (demand_line_filled) {
                    num_useless_prefetches_accesses += num_prefetch_hits_in_demand_line;
        ...
        int total_prefetch_in_line = num_useful_prefetches_accesses + num_useless_prefetches_accesses;
        if (total_prefetch_in_line != item.second.prefetches_in_accesses) {
            std::cout << "Debug issued prefetch discrepancy" << std::endl;
            std::cout << "num_useful_prefetches_accesses: " << num_useful_prefetches_accesses << " num_useless_prefetches_accesses: " << num_useless_prefetches_accesses << std::endl;
            std::cout << "num prefetches in accesses: " << item.second.prefetches_in_accesses << std::endl;
            if (item.second.current_index+1 != item.second.accesses.size()) {
                std::cout << "size not equal! current_index: " << item.second.current_index + 1 << " vector size: " << item.second.accesses.size() << std::endl;
            }
            print_accesses(item.second.accesses);
        }
        num_useful_prefetches += num_useful_prefetches_accesses;
        num_useless_prefetches += num_useless_prefetches_accesses;

        
    }
    // sanity check for issued prefetches
    int total = num_useful_prefetches + num_useless_prefetches;
    if (total != num_total_prefetches) {
        std::cout << "total != num total prefetches!" << std::endl;
        std::cout << "num useful prefetches + num useless prefetches: " << total 
            << " num total prefetches: " << num_total_prefetches << std::endl;
        
    }
    // accuracy for issued prefetches
    std::cout << "Issued Prefetch Accuracy: " << (double) num_useful_prefetches / (double) (num_useful_prefetches + num_useless_prefetches) << std::endl;

    // accuracy for prefetch fills
    int total_misses = num_pm_ph_d_intervals + num_pm_ph_e_intervals;

    if (total_misses != num_total_prefetch_fills) {
        std::cout << "total misses != num total prefetches fills!" << std::endl;
        std::cout << "num pm ph* d + num pm ph* e: " << total_misses 
            << " num total prefetches fills: " << num_total_prefetch_fills << std::endl;
    }

    // accuracy for issued prefetches
    std::cout << "Prefetch Fill Accuracy: " << (double) num_pm_ph_d_intervals / (double) (num_pm_ph_d_intervals + num_pm_ph_e_intervals) << std::endl;
}

void Tracer::final_prefetcher_accuracy(void) {
    uint64_t accurate = 0;
    double weighted_accuracy = 0.0;
    uint64_t total = 0;
    for (auto& item : ip_accuracy_map) {
        accurate += item.second.first;
        weighted_accuracy += ((double) pc_freq_map[item.first] / (double) total_pc_freq) * item.second.first; 
        total += item.second.second;
    }

    cout << " ip map final accuracy: " << (double) accurate / (double) total << endl;
    cout << " ip map final weighted accuracy: " << weighted_accuracy / (double) total << endl;

    cout << "prefetcher accuracy from ChampSim: " << (double) useful_prefetch_total / (double) prefetch_miss_total << endl;

    cout << "total: " << prefetch_miss_total << endl;
    cout << "accurate: " << useful_prefetch_total << endl;

    accurate_prefetch_total = accurate;

}

void Tracer::final_prefetcher_lateness(void) {
    cout << "late total: " << late_total << endl;
    cout << "useful_prefetch_total: " << useful_prefetch_total << endl;
    cout << "lateness: " << (double) late_total / (double) useful_prefetch_total << endl;
}

void Tracer::print_memory_request_total(void) {
    cout << "memory_request_total: " << memory_request_total << endl;
}

void Tracer::print_dram_stats(void) {
    cout << "num dram requests: " << memory_request_total << endl;

    uint64_t dram_total_latency = 0;
    for (auto it = dram_latencies_vector.begin(); it != dram_latencies_vector.end(); it++) {
        dram_total_latency += *it;
    }
    cout << "average dram latency: " << (double) dram_total_latency / (double) memory_request_total << endl;

    double median = 0;
    if (dram_latencies_vector.size() > 0) {
      sort(dram_latencies_vector.begin(), dram_latencies_vector.end());
      if (dram_latencies_vector.size() % 2 != 0) {
        median = (double) dram_latencies_vector[dram_latencies_vector.size() / 2];
      }
      else {
        median = (double) ((double) dram_latencies_vector[dram_latencies_vector.size() / 2] 
                  + dram_latencies_vector[(dram_latencies_vector.size() / 2) + 1]) / 2.0;
      }
      cout << "median dram latency: " << median << endl;
    }
}
