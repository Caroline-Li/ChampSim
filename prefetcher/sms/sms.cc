...
void SMSPrefetcher::invoke_prefetcher(uint64_t pc, uint64_t address, uint8_t cache_hit, uint8_t type, uint64_t instr_id, uint64_t curr_cycle,
                                 std::vector<uint64_t>& pref_addr, std::vector<bool>& pref_level, std::vector<int>& offsets)
{
	...
	else
	{
		...
		else
		{
			...
			generate_prefetch(pc, address, page, offset, pref_addr, pref_level, offsets);
			if(knob::sms_enable_pref_buffer)
			{
				buffer_prefetch(pref_addr, offsets);
				pref_addr.clear();
                offsets.clear();
                pref_level.clear();
			}
	...
	if(knob::sms_enable_pref_buffer)
	{
		issue_prefetch(pref_addr, pref_level, offsets);
	}
}
...
int SMSPrefetcher::generate_prefetch(uint64_t pc, uint64_t address, uint64_t page, uint32_t offset, vector<uint64_t> &pref_addr, vector<bool>& pref_level, vector<int>& offsets)
{
...
void SMSPrefetcher::buffer_prefetch(vector<uint64_t> pref_addr, vector<int>& offsets)
{
	...
	for(uint32_t index = 0; index < pref_addr.size(); ++index)
	{
        ...
        pref_offset_buffer.push_back(offsets[index]);
		count++;
	}
...
void SMSPrefetcher::issue_prefetch(vector<uint64_t> &pref_addr, vector<bool>& pref_level, vector<int>& offsets)
{
	uint32_t count = 0;
	while(!pref_buffer.empty() && count < knob::sms_pref_degree)
	{
        ...
        pref_level.push_back(FILL_L2);
        offsets.push_back(pref_offset_buffer.front());
...
void SMSPrefetcher::register_fill(uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint64_t curr_cycle) {
    return;
}