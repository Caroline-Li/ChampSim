...
const char* MapWeightTypeString[] = {"Global", "Per-action", "Per-state", "Per-state-action"};
...
string FeatureKnowledge::getWeightTypeString(WeightType weight)
{
  assert(weight < WeightType::NumWeightTypes);
  return MapWeightTypeString[(uint32_t)weight];
}
...
FeatureKnowledge::FeatureKnowledge(FeatureType feature_type, float alpha, float gamma, uint32_t actions, float weight, float weight_gradient,
                                   float weight_minimum, uint32_t num_tilings, uint32_t num_tiles, bool zero_init, uint32_t hash_type,
                                   int32_t enable_tiling_offset, WeightType weight_type)
    : m_feature_type(feature_type), m_alpha(alpha), m_gamma(gamma), m_actions(actions), m_init_weight(weight), m_weight_gradient(weight_gradient),
      m_weight_minimum(weight_minimum), m_hash_type(hash_type), m_num_tilings(num_tilings), m_num_tiles(num_tiles),
      m_enable_tiling_offset(enable_tiling_offset ? true : false), m_weight_type(weight_type)
{
...
float FeatureKnowledge::retrieveQ(State* state, uint32_t action)
{
  ...
  // Get weight
  uint32_t weight_index = get_weight_index(m_weight_type, state, action);
  return get_weight(weight_index) * q_value;
}
...
void FeatureKnowledge::updateQ(State* state1, uint32_t action1, int32_t reward, State* state2, uint32_t action2)
{
  ...
  for (uint32_t tiling = 0; tiling < m_num_tilings; ++tiling) {
    ...
    // cout << "[DEBUG] addr=" << hex << (state1->address >> LOG2_BLOCK_SIZE) + knob::pythia_actions[action1]
    //      << " a1=" << dec << knob::pythia_actions[action1]
    //      << " q1=" << Qsa1_old
    //      << " a2=" << knob::pythia_actions[action2]
    //      << " q2=" << Qsa2
    //      << " R=" << reward
    //      << " q1'=" << Qsa1 << endl;
