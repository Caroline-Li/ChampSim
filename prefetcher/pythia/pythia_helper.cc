...
extern bool pythia_enable_pt_address_compression;
...
extern bool pythia_enable_dyn_degree_detector;
...
const char* MapRewardTypeString[] = {"none",        "incorrect",         "correct_untimely",         "correct_timely",        "out_of_bounds",
                                     "tracker_hit", "incorrect_lowconf", "correct_untimely_lowconf", "correct_timely_lowconf"};
...
{
  uint64_t value = 0;
  switch (knob::pythia_state_type) {
  case 1: /* Only PC */
    // return (uint32_t)(pc % knob::pythia_max_states);
    value = pc;
    break;
  ...
  case 2: /* PC+ offset */
    ...
    value = value << 6;
    value = value + offset;
    // value = value % knob::pythia_max_states;
    // return value;
    break;
  ...
  assert(hashed_value < knob::pythia_max_states);
...
{
  uint32_t value = 0;
  switch (knob::pythia_state_hash_type) {
  case 1: /* simple modulo */
    value = (uint32_t)(key % knob::pythia_max_states);
    break;

  case 2:
    value = HashZoo::jenkins((uint32_t)key);
    value = (uint32_t)(value % knob::pythia_max_states);
    break;
...
void Pythia_STEntry::update(uint64_t page, uint64_t pc, uint32_t offset, uint64_t address)
{
  assert(this->page == page);

  /* insert PC */
  if (this->pcs.size() >= knob::pythia_max_pcs) {
    this->pcs.pop_front();
  }
  ...
  if (!this->offsets.empty()) {
    ...
    if (this->deltas.size() >= knob::pythia_max_deltas) {
  ...
  /* insert offset */
  if (this->offsets.size() >= knob::pythia_max_offsets) {
  ...
  if (knob::pythia_print_trace) {
    fprintf(stdout, "[TRACE] page %16lx| pc %16lx| offset %2u\n", page, pc, offset);
  }
}

uint32_t Pythia_STEntry::get_delta_sig()
{
...
uint32_t Pythia_STEntry::get_delta_sig2()
{
  uint32_t curr_sig = 0;
...
uint32_t Pythia_STEntry::get_pc_sig()
{
  uint32_t signature = 0;
...
uint32_t Pythia_STEntry::get_offset_sig()
{
  uint32_t signature = 0;
...
void Pythia_STEntry::track_prefetch(uint32_t pred_offset, int32_t pref_offset)
{
...
void Pythia_STEntry::insert_action_tracker(int32_t pref_offset)
{
  // bool found = false;
  ...
  if (it != action_tracker.end()) {
    ...
    if (action_tracker.size() >= knob::pythia_action_tracker_size) {
...
bool Pythia_STEntry::search_action_tracker(int32_t action, int32_t& conf)
{
  conf = 0;
...
void PythiaRecorder::record_access(uint64_t pc, uint64_t address, uint64_t page, uint32_t offset, uint8_t bw_level)
{
...
void PythiaRecorder::record_trigger_access(uint64_t page, uint64_t pc, uint32_t offset) { unique_trigger_pcs.insert(pc); }

void PythiaRecorder::record_access_knowledge(Pythia_STEntry* stentry)
{
...
void PythiaRecorder::dump_stats()
{
...
{
  bool to_print = true;
  if (knob::pythia_print_access_debug_pc) {
    if (stentry->unique_pcs.find(knob::pythia_print_access_debug_pc) == stentry->unique_pcs.end()) {
      to_print = false;
    } else {
      debug_print_count++;
      if (debug_print_count > knob::pythia_print_access_debug_pc_count) {
        to_print = false;
      }
    }
  }
...
string print_weight_type(uint32_t weight_type) { return FeatureKnowledge::getWeightTypeString((WeightType)weight_type); }

string print_pooling_type(uint32_t pooling_type) { return LearningEngineFeaturewise::getPoolingTypeString(pooling_type); }