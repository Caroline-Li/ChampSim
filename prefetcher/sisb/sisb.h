...
void sisb_l2c_prefetcher_initialize() {

}

void sisb_l2c_prefetcher_operate(uint64_t addr, uint64_t pc, uint8_t cache_hit, uint8_t type, CACHE *parent) {
    ...
    uint32_t count = 0;
    while (count++ < SISB_DEGREE && pred != 0) {
        // issue prefetch
...
void sisb_l2c_prefetcher_cache_fill(uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr) {
    uint64_t addr_B = addr >> LOG2_BLOCK_SIZE;
...
bool flag = false;
void sisb_l2c_prefetcher_final_stats() {
